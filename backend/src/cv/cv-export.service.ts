...
export class CvExportService {
  ...
  async exportCvAsPdf(
    ...
    const fullName = cvData.fullName ?? userRecord?.fullName ?? '';
    ...
    const summary =
      ...
      '';
    const skills: string[] =
      cvData.intelligence?.improvedCv?.optimizedSkills ?? cvData.skills ?? [];
    const experience = cvData.experience ?? [];
    const education = cvData.education ?? [];

    let html: string;
    if (template === 'classic') {
      html = this.renderClassic({
        fullName,
        email,
        title,
        phone,
...
<body>
  ...
  private renderClassic(data: {
    ...
    const skillsHtml = skills
      .map(
        (s) =>
          `<span style="display:inline-block;border:1px solid #9CA3AF;border-radius:3px;padding:2px 8px;font-size:11px;margin:2px 3px 2px 0;color:#374151;">${this.escapeHtml(s)}</span>`,
      )
      .join('');
...
<body>
  <div class="header">
    <h1>${this.escapeHtml(fullName)}</h1>
    ${title ? `<div class="subtitle">${this.escapeHtml(title)}</div>` : ''}
    <div class="contact-row">${[email, phone, location]
      .filter(Boolean)
...
<body>
  ...
  <div class="contact">${[email, phone, location]
    .filter(Boolean)
    .map((v) => this.escapeHtml(v))
    .join(' | ')}</div>
