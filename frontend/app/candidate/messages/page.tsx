...
import { candidateConversations } from "@/src/data/messages"

export default function CandidateMessagesPage() {
  const [selectedConv, setSelectedConv] = useState<string>(candidateConversations[0]?.id ?? "")
  const [newMessage, setNewMessage] = useState("")

  const conversation = candidateConversations.find((c) => c.id === selectedConv)
      ...
      <DashboardLayout>
        <div className="space-y-4">
          <div>
            ...
            <p className="text-muted-foreground">تواصل مع مسؤولي التوظيف</p>
          </div>
                        ...
                        {conversation.messages.map((msg) => (
                          <div key={msg.id} className={`flex gap-3 ${msg.senderId === "1" ? "flex-row-reverse" : ""}`}>
                            ...
                            <div className={`max-w-[70%] rounded-lg p-3 ${
                              msg.senderId === "1" ? "bg-primary text-primary-foreground" : "bg-secondary text-foreground"
                            }`}>
                              <p className="text-sm">{msg.content}</p>
                              <p className={`mt-1 text-xs ${msg.senderId === "1" ? "text-primary-foreground/70" : "text-muted-foreground"}`}>
